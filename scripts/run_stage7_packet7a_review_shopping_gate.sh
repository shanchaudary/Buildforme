...
if [ "$apply_status" -eq 0 ]; then
  ...
  python -m py_compile buildforme/execution_store.py buildforme/execution_service.py tests/test_stage7_review_authority.py 2>&1 | tee -a "$report"
  ...
  echo '== focused review authority tests ==' | tee -a "$report"
  ...
  if [ "$stage7_status" -eq 0 ] && [ "$mutation_status" -eq 0 ]; then
    focused_status=0
  else
    focused_status=1
  fi
...
if [ "$correction_status" -eq 0 ] && [ "$apply_status" -eq 0 ] && [ "$syntax_status" -eq 0 ] && [ "$focused_status" -eq 0 ] && [ "$full_status" -eq 0 ] && [ "$policy_status" -eq 0 ] && [ "$constitution_status" -eq 0 ]; then
  ...
  git commit -m "Block Stage 7 review shopping"
  git push origin HEAD:stage-7-independent-multi-agent-review-loop
  exit 0
fi

cp "$report" /tmp/stage7_packet7a_review_shopping_validation.txt
printf '\n== validation report tail ==\n'
tail -n 200 "$report" || true
git restore .
git clean -fd buildforme tests docs scripts
restore_original_ci
cp /tmp/stage7_packet7a_review_shopping_validation.txt "$report"
git add .github/workflows/ci.yml
git add -f "$report"
git commit -m "Record review-shopping validation failure"
git push origin HEAD:stage-7-independent-multi-agent-review-loop
exit 1
