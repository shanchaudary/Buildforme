...
if [ "$apply_status" -eq 0 ]; then
  ...
  echo '== focused Packet 7B and authority regressions ==' | tee -a "$report"
...
if [ "$fix_status" -eq 0 ] && [ "$apply_status" -eq 0 ] && [ "$syntax_status" -eq 0 ] && [ "$focused_status" -eq 0 ] && [ "$full_status" -eq 0 ] && [ "$policy_status" -eq 0 ] && [ "$constitution_status" -eq 0 ]; then
  ...
  git commit -m "Add Stage 7 automated blind reviewer execution"
...
restore_original_ci
cp /tmp/stage7_packet7b_validation.txt "$report"
git add .github/workflows/ci.yml
git add -f "$report"
git commit -m "Record Stage 7 Packet 7B validation failure"
git push origin HEAD:stage-7-independent-multi-agent-review-loop
exit 1
